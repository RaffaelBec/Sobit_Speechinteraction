...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<String[]> nerTaggedWords = List.of(
                ...
                new String[]{"des", "O"},
                new String[]{"nach", "B-Time"},
                new String[]{"##sten", "B-Time"},
                new String[]{"einsatz", "B-Task"},
                new String[]{"anrufen", "B-Action"}
        );
    ...
    private static void setupSampleData() {
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            stmt.execute("INSERT INTO Client (ClientId, FirstName, LastName) VALUES (3, 'Kilian', 'Bauer')");
            stmt.execute("INSERT INTO Client (ClientId, FirstName, LastName) VALUES (4, 'Christian', 'Reeh')");
            // Insert assignments
            stmt.execute("INSERT INTO Assignment (AssignmentId, AssignmentTypeId, ClientId, EmployeeId, TargetTimeStart, TargetTimeEnd) " +
                    "VALUES (1, 1, 1, 1, DATETIME('now','+1 hour','localtime'), DATETIME('now','+5 hour','localtime'))");
            ...
            stmt.execute("INSERT INTO Assignment (AssignmentId, AssignmentTypeId, ClientId, EmployeeId, TargetTimeStart, TargetTimeEnd, ActualTimeStart) " +
                    "VALUES (2, 1, 2, 1, DATETIME('now','localtime'), DATETIME('now', '+5 hour'), DATETIME('now','localtime'))");
            stmt.execute("INSERT INTO Assignment (AssignmentId, AssignmentTypeId, ClientId, EmployeeId, TargetTimeStart, TargetTimeEnd) " +
                    "VALUES (3, 1, 3, 1, DATETIME('now','+2 hour', 'localtime'), DATETIME('now', '+5 hour','localtime'))");

            stmt.execute("INSERT INTO Assignment (AssignmentId, AssignmentTypeId, ClientId, EmployeeId, TargetTimeStart, TargetTimeEnd) " +
                    "VALUES (4, 1, 4, 1, DATETIME('now', '-1 day', 'localtime'), DATETIME('now', '-1 day', '+5 hour','localtime'))");

            stmt.execute("INSERT INTO Person (PersonId, FirstName, LastName, PhoneNr) VALUES (3, 'Hana', 'Horvath', '555-555-5555')");
            ...
            stmt.execute("INSERT INTO PersonRelationship (RelationshipId, RelationshipTypeId, ClientId, PersonId, DateStart) VALUES (4, 2, 2, 4, DATE('now'))");
